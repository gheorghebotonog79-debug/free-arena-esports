...
export function ServerFaq({ label, locale, page, title }: ServerFaqProps) {
  ...
  return (
    <section id="faq" className="neon-section px-4 pb-14 sm:px-6 lg:px-8">
      <div className="mx-auto w-full max-w-7xl">
        <p className="text-xs font-black uppercase tracking-[0.2em] text-arena-green">
          {label}
        </p>
        <h2 className="neon-title neon-text-pulse mt-3 font-display text-3xl font-black uppercase text-white">
          {title}
        </h2>
        <div className="mt-6 grid gap-3">
          {faq.map((item) => (
            <details
              key={item.question}
              className="premium-card glass-panel neon-hover group rounded-lg p-5"
            >
