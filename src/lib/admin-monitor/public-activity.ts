...
function emptyAdminActivityList(month: string) {
  return {
    admins: [],
    month,
    totals: {
...
export async function getPublicAdminActivityList(month = getMonthKey(new Date())) {
  try {
    ...
    if (steamIds.length === 0) {
      return emptyAdminActivityList(month);
    }
    ...
    console.error("[admin-activity] Public list unavailable", error);
...
export async function getPublicAdminActivityDetail(steamId: string) {
  try {
    ...
    console.error("[admin-activity] Public detail unavailable", error);
